...
class RobotSimulator:
    ...
    def __init__(self, time_step, robot):
        ...
        self.init(np.array([-np.pi, 0.0]), None, True)
        
        self.tau_c = np.zeros(na)   # Coulomb friction torque
        self.simulation_type = 'euler'
        self.set_coulomb_friction(0*np.ones(2))
    ...
    def compute_forces(self, compute_data=True):
        '''Compute the contact forces from q, v and elastic agent'''
        if compute_data:
